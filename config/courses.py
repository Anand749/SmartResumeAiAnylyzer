...
COURSES_BY_CATEGORY = {
    ...
    "Cybersecurity": {
        ...
        "Penetration Tester": [
            ...
            ["Advanced Penetration Testing (OSCP)", "https://www.offensive-security.com/pwk-oscp/"]
        ]
    },
    "UI/UX Design": {
        "UI Designer": [
...
def search_courses(keyword):
    """Search courses by keyword (e.g., 'React', 'Python')"""
    results = {}
    for category, roles in COURSES_BY_CATEGORY.items():
        for role, courses in roles.items():
            matches = [c for c in courses if keyword.lower() in c[0].lower()]
            if matches:
                results.setdefault(category, {})[role] = matches
    return results

def count_courses(role_name):
    """Count courses for a role (e.g., 'Frontend Developer')"""
